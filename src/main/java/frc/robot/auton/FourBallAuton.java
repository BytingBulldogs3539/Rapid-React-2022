...
public class FourBallAuton extends SequentialCommandGroup {
    public FourBallAuton(DriveSubsystem driveSub, IntakeSubsystem intakeSubsystem, PneumaticsSubsystem pneumaticsSubsystem, ShooterSubsystem shooterSubsystem) {
        super(new Command[] {new ShooterCommand(shooterSubsystem, intakeSubsystem, false, 2800,2800, 3000).withTimeout(1), //Shoots for 4 seconds
            new ParallelRaceGroup( TrajectoryCommandGenerator
                .getMotionCommand((new SimplePathBuilder(new Vector2(0.0, 0.0), Rotation2.ZERO))
                        ...
                        new ShooterCommand(shooterSubsystem, intakeSubsystem, false, 2800,2800, 3000).withTimeout(1.5),

                        new ParallelRaceGroup( TrajectoryCommandGenerator
                        .getMotionCommand((new SimplePathBuilder(new Vector2(0.0, -1.7), Rotation2.fromDegrees(-395)))
                                .lineTo(new Vector2(-1, -.5), Rotation2.fromDegrees(-180))
                                .build(),RobotContainer.constants.getDriveConstants().getConstraints(), false, driveSub),
                                new IntakeCommand(1.0, 0, intakeSubsystem, pneumaticsSubsystem, shooterSubsystem).withTimeout(10))}); // Shoots after it has finished driving
    }
}