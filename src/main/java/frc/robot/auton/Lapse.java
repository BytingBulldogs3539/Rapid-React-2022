...
public class Lapse extends SequentialCommandGroup {
    public Lapse(DriveSubsystem driveSub) {
        super(new Command[] { TrajectoryCommandGenerator
                .getMotionCommand((new SimplePathBuilder(new Vector2(0.0, 0.0), Rotation2.ZERO))
                        .lineTo(new Vector2(1, 0), Rotation2.ZERO)
                        .arcTo(new Vector2(2, -1), new Vector2(1, -1), Rotation2.ZERO)
                        .lineTo(new Vector2(2, -3), Rotation2.ZERO)
                        // This point for the big curve is divided into 2 to allow it to take the path that we want it to.
